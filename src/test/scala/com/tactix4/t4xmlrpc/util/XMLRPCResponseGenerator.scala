...
object XMLRPCResponseGenerator extends Properties("XML-RPC Reponse generator") {
 ...
 implicit def arbitraryXmlRpcResponse: Arbitrary[List[Content]] =
...
//  } yield <methodResponse><params>{p}</params></methodResponse>
   ...
   */
  def randomValidFaultGen: Gen[String] = for {
    fs <- rFaultStruct
  }yield s"<methodResponse><fault>$fs</fault></methodResponse>"
...
//  }yield <methodResponse><fault>{ifs}</fault></methodResponse>
  ...
  def rB64: Gen[String]         = for {b64 <- arbString.arbitrary}  yield s"<value><base64>${new sun.misc.BASE64Encoder().encode(b64.getBytes)}</base64></value>"
